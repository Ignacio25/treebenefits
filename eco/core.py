...
def sum_factor_and_conversion(*args):
    ...
    """
    return list(map(sum_ignore_none, zip(*args)))
...
class Benefits(object):
    ...
    def _get_data(self, region, factor):
        ...
        if data_file not in self._factor_cache:
            ...
            dbh_breaks = np.array(list(map(safe_float, self._strip_trailing_empty_cells(dbh_breaks_dirty))))
            datarows = alldata[1:]
            data = {}
            ...
            for row in datarows:
                if len(row[0]) > 0:
                    ...
                    data[row[0]] = np.array(list(row_data))
    ...
    def get_factor_and_conversion_for_trees(self, region, factor, species_codes_and_dbh):
    ...
    def get_energy_conserved(self, region, species_codes_and_dbh):
        ...
        # 1000s of BTU?
        nat_gas_kbtu, nat_gas_converted = self.get_factor_and_conversion_for_trees(region, 'natural_gas', species_codes_and_dbh)
        nat_gas_kwh = nat_gas_kbtu * Benefits.WATTS_PER_BTU

        energy_kwh, energy_converted = self.get_factor_and_conversion_for_trees(region, 'electricity', species_codes_and_dbh)
        return energy_kwh
    ...
    def _get_lbs(self, region, species_codes_and_dbh, factor):
        ...
        return ( factor_value_kg, converted_factor_value_kg if converted_factor_value_kg else None )
    ...
    def get_co2_stats(self, region, species_codes_and_dbh):
        ...
        data['reduced'] = sum_factor_and_conversion(data['sequestered'], data['avoided'])
    ...
    def get_air_quality_stats(self, region, species_code_and_dbh):
        ...
        data = {
            ...
            'nox': sum_factor_and_conversion( get_lbs('aq_nox_dep'), get_lbs('aq_nox_avoided') ),
            'pm10': sum_factor_and_conversion( get_lbs('aq_pm10_dep'), get_lbs('aq_pm10_avoided')),
            'sox': sum_factor_and_conversion( get_lbs('aq_sox_dep'), get_lbs('aq_sox_avoided')),
            'voc': get_lbs('aq_voc_avoided'),
            'bvoc': get_lbs('bvoc')
        }
        data['improvement'] = sum_factor_and_conversion( data['ozone'], data['nox'], data['pm10'], data['sox'], data['voc'], data['bvoc'])
        return data
